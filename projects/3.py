...
if case_road == "left":
  ...
  lake = input(
      'You come to a lake.There is an island  in between the lake.Type "wait" to wait for a boat or type "swim" to swim across.\n'
  )
  lake_case = lake.lower()
  if lake_case == "wait":
    door = input(
        'You arrive at the island unharmed.There is a house with 3 doors.One red, oneyellow , one blue.Which color do you chose?,R, Y , B '
    )
    if door_case == "R":
